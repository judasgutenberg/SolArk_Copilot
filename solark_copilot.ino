...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if(goodDataMode) {
      ...
      if (incomingByte == '\r'){
        //transmissionString = goodData;
        int packetSize = goodData.length();
        transmissionString =  parseInverterData(goodData) + "*" + (String)packetSize;
        transmissionString = transmissionString + "*" + additionalPowerData(); //*volta*ampa*voltb*ampb after packetSize
        if(millis() - lastDataLogTime > data_logging_granularity * 1000 || lastDataLogTime == 0) {
          ...
          if(sensor_id > -1) {
            ...
            transmissionString +=  additionalSensorData;
        ...
        transmissionString = transmissionString + (int)lastCommandId + "**" + (int)localSource + "*" + ipAddressToUse + "*" + (int)requestNonJsonPinInfo + "*1*" + changeSourceId;
        transmissionString = transmissionString +  "|" + joinMapValsOnDelimiter(pinMap, "*");
        transmissionString = transmissionString +  "|";
        
        feedbackPrint(packetSize);
        feedbackPrint("\n");
        feedbackPrint(transmissionString); 
        feedbackPrint("\n");
        if(packetSize == 745) {
          sendRemoteData(transmissionString, "saveLocallyGatheredSolarData", includesWeatherData);
        } else {
          sendRemoteData(transmissionString, "suspectLocallyGatheredSolarData", includesWeatherData);
        }
        goodData = "";
        goodDataMode = false;
      } else {
    ...
    if(endsWith(serialContent, startValidIndication)){ //!goodDataMode && 
      //transmissionString += "foundone\n";
      //postData(transmissionString + "\n" + serialContent);
      serialContent = "";
      totalSerialChars = 0;
      goodDataMode = true;   
    }
...
void handleInverterData() {
 String s = transmissionString; //Read HTML contents
